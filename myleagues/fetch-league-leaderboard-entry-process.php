...
// --- FETCH DETAILS FOR SPECIFIED LEAGUE LEADERBOARD ENTRY 
...
if (isset($_SESSION["user_id"])) {                      // Check user is set
    ...
    if (isset($data["leaderboardId"]) && isset($data["userId"])) {          // Check leaderboard id and user id for selected entry are set
        ...
        $sql = "SELECT * FROM league_leaderboards_entries WHERE leaderboard_id = ? AND user_id = ?";          
        ...
        $entry = $result->fetch_assoc();        // Store fetched entry
